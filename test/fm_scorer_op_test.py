...
from tffm.fm_ops import fm_scorer
import random
...
class FmScorerOpTest(tf.test.TestCase):
    ...
    def testBasic(self):
        ...
        feature_vals = [[9.0, 2.0, 3.6, 4.0, -2.2, -10.7],
                        [4.0, -3.4, 2.0, 0, 0, 0]]
        labels = [1.0, -1.0]
        factor_lambda = 1.2
        bias_lambda = 0.4
        ...
        for i in range(vocabulary_size):
            params.append([random.uniform(0.01, 0.02)
                           for j in range(factor_num + 1)])
        ...
        factor_map = tf.slice(
            params_var, [
                0, 1], [
                vocabulary_size, factor_num])
        bias_map = tf.reshape(
            tf.slice(
                params_var, [
                    0, 0], [
                    vocabulary_size, 1]), [vocabulary_size])
        ...
        tf_pred = 0.5 * tf.reduce_sum(factor_sum * factor_sum, [1]) \
            - 0.5 * tf.reduce_sum(fvals * fvals * factors * factors, [1, 2]) \
            + tf.reduce_sum(biases * feature_vals, [1])
        tf_reg = 0.5 * (
            factor_lambda * tf.reduce_sum(factors * factors)
            + bias_lambda * tf.reduce_sum(biases * biases)
        )
        tf_cost = tf.reduce_sum(
            tf.squared_difference(
                labels, tf_pred)) + tf_reg
        tf_grad = tf.gradients(tf_cost, params_var)

        my_pred, my_reg = fm_scorer(
            flat_feature_ids, params_var, flat_feature_vals,
            flat_feature_poses, factor_lambda, bias_lambda
        )
        my_cost = tf.reduce_sum(
            tf.squared_difference(
                labels, my_pred)) + my_reg
